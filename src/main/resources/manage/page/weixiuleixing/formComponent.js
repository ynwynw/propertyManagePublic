Vue.component('weixiuleixing-form', {
template:`
<template>
    <div>
        <el-dialog
...
methods: {
    ...
    getInfo(){
        http.get(`weixiuleixing/info/${this.id}`).then(res=>{
    ...
    save(){
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('weixiuleixing/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`weixiuleixing/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
